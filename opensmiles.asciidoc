...
========================
Craig A. James
v1.0,2012-29-09: Current specification
...
McLeod and Peter's quip captures the deficiencies of SMILES well: if you
...
[options="header",frame="topbot",grid="rows",width="90%",cols="1,4"]
|============================
...
|                            _aromatic_symbols_ ::= `'c'` \| `'n'` \| `'o'` \| `'p'` \| `'s'` \| `'se'` \| `'as'`
2+| **CHIRALITY**
| <<chirality,Chirality>>  | _chiral_ ::= `'@'` \| `'@@'` \| `'\@TH1'` \| `'\@TH2'` \| `'\@AL1'` \| `'\@AL2'` \| `'\@SP1'` \| `'\@SP2'` \| `'\@SP3'` \| `'\@TB1'` \| `'\@TB2'` \| `'\@TB3'` \| ... \| `'\@TB20'` \| `'\@OH1'` \| `'\@OH2'` \| `'\@OH3'` \| ... \| `'\@OH30'`
2+| **HYDROGENS**
| <<hydrogens,Hydrogens>>  | _hcount_ ::= `'H'` \| `'H'` _DIGIT_
2+| **CHARGES**
| <<charge,Charge>>        | _charge_ ::= `'-'` \| `'-'` _DIGIT_ \| `'+'` \| `'+'` _DIGIT_ \| `'--'` *deprecated*  \| `'++'` *deprecated*
2+| **ATOM CLASS**
...
|                            _dot_ ::= `'.'`
2+| **SMILES STRINGS**
.2+|                       | _smiles_ ::= _chain_ _terminator_
...
http://www.iupac.org/reports/provisional/abstract04/RB-prs310804/TableI-3.04.pdf[111 valid atomic symbols]
...
| `[Cu+2]`  | coppor cation         | `+2` charge, `H0` implied
| `[Cu++]`  | copper anion          | `+2` charge, `H0` implied
|==============================================================

_Question: do we need to allow two-digit charges, like `10`?_

Isotopes
^^^^^^^^

Isotopic specification is placed inside the square brackets for an atom
preceeding the atomic symbol; for example:
...
In a SMILES string such as "C1CCCCC1", the first occurance of a ring-closure
...
It is permissable to re-use ring-closure numbers.  Once a particular number
...
Two-digit ring numbers are permitted, but must be preceeded by the percent
...
* The number of availabile "shared" http://en.wikipedia.org/wiki/Pi_electron[&#960; electrons]
...
In an aromatic system, all of the aromatic atoms must be sp:sup:`2` hybridized, and the
...
be explicitely represented.  For example:
...
A chiral center can be written starting from any of its neighbor atoms, and the choice of
...
when the fluorine atom moved from the left to the right side of the allenal carbon atom.
...
to which the chirality refers are at the ends of the allenal system.  For example:
...
The shorthand notations `'@'` and `'@@'` correspond to clockwise and
...
hese "relaxed" SMILES rules should only be allowed when the user
...
`'@'` for Cis/Trans around Double Bonds
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
`'\'`.  In a cyclo-ene (_name??_) ring with an even number of double bonds, it is
...
| image:depict/cyclooctatetraene.gif[] | `C/1=C/C=C\C=C/C=C\1` | Illegal SMILES
|==============================================================================

The SMILES above is illegal because the first and second `C1` have
...
fact the atoms form a circle.

The proposed syntax for cis/trans configurations uses the `'@'` symbol on
...
Note that in all cases, cis and trans are easy for the chemist to
...
This proposed form of cis/trans specification using `'@'` and `'@@'` does not suffer from the
theoretical flaw illustrated above:
...
Note that the first allenal carbon must be represented as `'@'` since the
...
The use of the non-aromatic lowercase symbol is a shorted form with improved intelligibilty that
...
* http://www.metamolecular.com[ChemWriter]
* JME
* CACTVS
* ISISDraw?
* ChemDraw
